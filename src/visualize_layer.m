...
%
%   show_names
%   Options: (0) shows no Tx and Rx names; (1, default) shows the Tx name; (2) shows the Tx and Rx name
%
%   create_new_figure
...
plot3( h_layout.tx_position(1,tx(1)),h_layout.tx_position(2,tx(1)),h_layout.tx_position(3,tx(1)),...
    ['+' color],'Linewidth',3,'Markersize',15 );
hold on
...
plot3( pos_tx_ant(1,:),pos_tx_ant(2,:),pos_tx_ant(3,:),...
    ['^' color],'Linewidth',1,'Markersize',10 );
...
for m = 1:numel(tx)
    plot3( [h_layout.tx_position(1,tx(m)),h_layout.tx_position(1,tx(m))] , ...
        ...
        [0,h_layout.tx_position(3,tx(m))] ,['--' color] );
...
for m = 1:numel(rx)
    ...
    if m>1
        ...
        plot3( pos(1,:),pos(2,:),pos(3,:), 'Linewidth',4 );
        %plot3( pos_rx_ant(1,:),pos_rx_ant(2,:),pos_rx_ant(3,:),...
           % 'vb','Linewidth',1,'Markersize',10 );
    end
...
%         end
    if show_names == 2
        text(h_layout.track(1,rx(m)).initial_position(1),...
            ...
            h_layout.track(1,rx(m)).name, 'fontWeight','bold') ;
    end
end
